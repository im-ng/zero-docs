...
load and when its dependencies misbehave. This page consolidates them; individual
feature pages (Configuration, Observability, PubSub, KV Store) carry the full key
and API references.
...
timed-out calls. A breaker opens after `failure_threshold` consecutive failures, stays
open for `cooldown_ms`, then allows `half_open_trials` probe requests before closing
again. While open, calls return `error.CircuitOpen`.
...
after a broker drop. Handler failures are retried up to 3× with a 500 ms backoff.
...
breaker. See [Http Service → Resilience](/http-service#resilience-timeouts--retries).
