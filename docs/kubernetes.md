...
This guide shows how to run a **zero** application on Kubernetes. The framework ships a
production-oriented Helm chart — **Bee** (chart `version: 0.2.0` targets Zero
`appVersion: 0.16.0`) — in its [`helm/`](https://github.com/im-ng/zero/tree/experimental/helm)
directory. You containerize your app once, push the image, then drive the whole deployment
...
prefix**, so a release named `kwant` produces objects literally named `kwant`
...
under `examples/zero-basic/`. It builds natively for **musl** on Alpine (zig 0.16.0),
bundles the Alpine-specific `libduckdb.so`, and ships a minimal runtime image.

> Build from the **framework repo root** — the Dockerfile `COPY`s `build.zig`, `src`,
> `examples/zero-basic`, and `examples/zero-basic/libs/`, so the build context must be `.`.
...
seccomp profile blocks — hence `--security-opt seccomp=unconfined` on `podman run`. In
Kubernetes this is covered by the chart's `securityContext` / `podSecurityContext` values
...
`8000`, so when you deploy via the chart, set `app.port` (and the container's `HTTP_PORT`)
...
Embedded `configMap` / `secret` are named from `nameOverride` and auto-loaded — no manual
names to repeat:
...
This renders `envFrom: - configMapRef: {name: kwant-cm}, - secretRef: {name: kwant-secret}`.
...
`PrometheusRule` CR — this cluster runs standalone Prometheus, not the operator). Mount it
as a volume in your Prometheus deployment:
...
nodeAffinity:
  ...
  preferred: tika # preferred node label worker=tika
```

## Health, metrics & probes (zero 0.16)

Zero 0.16 serves its liveness endpoint at **`/.well-known/health`** (not `/health`). The
chart hardcodes that default in `app.healthPath` and wires it into the `livenessProbe`,
`readinessProbe`, and a `startupProbe` (the latter tolerates slow boots such as DB
migrations). Metrics are scraped from `app.metricsPort` at `app.metricsPath` (default
`/metrics`).

## Resource sizing (zero 0.16)

The httpz buffer pool was decoupled (PR #9), dropping steady-state RSS to **~16–65 MiB**,
so the chart default requests are `cpu: 100m / memory: 64Mi` (limits `500m / 256Mi`). Scale
requests up if you enable large SQLite/S3 workloads.
...
- Logging: `LOG_FORMAT` (`text`/`json`), `ZERO_LOG_TIMEZONE`.
- HTTP/runtime: `ZERO_REQUEST_TIMEOUT_MS`, `INBOUND_MAX_CONCURRENT`, `ZERO_HTTP_LARGE_BUFFER_*`.
- Inbound rate limiting: `RATE_LIMIT_*`.
- Datasources: `DB_*`, `REDIS_*`, `SQLITE_*`, `S3_*`, `FILE_STORE_*`.
- Outbound service clients: `SERVICE_<NAME>_*`.

## Full example

See [`values-override.yaml`](https://github.com/im-ng/zero/tree/experimental/helm/values-override.yaml)
for the `kwant` example, and
