...
This page shows how to build `zero` and its examples for different targets and CPU
baselines, plus how to debug when something goes wrong.

All commands assume you're using the **Zig 0.16.0** toolchain (`zero` sets
...
make small | fast | base  # complies to above mentioned modes
./zig-out/bin/basic       # run it directly
```

Most examples also expose a `run` step. For example, `zig build basic` runs the
server. To start clean, the framework `Makefile` ships a `clean` target. It wipes
every example's `.zig-cache`, `zig-out`, and `zig-pkg`.
...
The build uses `b.standardTargetOptions(.{})`. That lets you cross-compile any
...
It links musl statically, so the binary runs on any glibc or
musl host without a matching sysroot.

Cross-compiling to a **different OS** (say, building a Linux binary from macOS)
needs the target's libc sysroot.

## Baseline CPU builds (older processors)

Some hosts run on older processors (e.g. Intel **Xeon** families). They lack the
newer instruction sets (AVX-512, etc.) that Zig's default `native` CPU tuning
assumes. If you build for `native` on a modern dev machine, the binary may crash with
illegal-instruction errors on the older host.

::: danger
Never skip to verify the underlying processor that targeted. Since I was using
old machines, I ended up wasting hours to figure out this issue.
:::
...
```bash
make base        # zig build -Dcpu=baseline --release=safe --summary all
```

`baseline` emits code for the minimal feature set every target CPU supports. The same
artifact boots on both new and old hardware. Use it whenever you build on a recent
machine but deploy to older ones.
...
| `make small`   | `zig build --release=small --summary all`               | Smallest binary for production.            |
| `make base`    | `zig build -Dcpu=baseline --release=safe --summary all` | Baseline-CPU build for older hosts.        |
...
It also ships a few runtime introspection helpers. These help when a running app
misbehaves:
...
Python pretty-printer (`lldb_pretty_printers.py`) in its install directory. It renders
Zig values (slices, strings, optionals, errors, structs) in readable form instead of
raw bytes.
...
- **Structured logs** — set `LOG_LEVEL=debug` in `configs/.env` (or `LOG_FORMAT=json`
  for machine parsing) to surface framework internals. See [Logging](/logging).
- **Known-leak caveat** — the unit test run uses `std.testing.allocator`. A few paths
  leak, and the build exits non-zero even when every assertion passes. That's a known
...
| Examples fail to fetch `zero`                                   | Missing/incorrect `zig fetch` of the `experimental` branch.       | Re-run the install from `refs/heads/main` (see [Migrating to 0.16](/migrating-0-16)).                                                                                                                                       |
